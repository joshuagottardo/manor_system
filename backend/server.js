...
app.get('/api/rooms', async (req, res) => {
  try {
    const [rooms] = await pool.query('SELECT * FROM rooms ORDER BY sort_order ASC');
    
    // Per ogni stanza, prendiamo i dispositivi
    // (In un progetto gigante si farebbe una JOIN, ma così è più leggibile per ora)
    for (let room of rooms) {
      const [devices] = await pool.query('SELECT * FROM devices WHERE room_id = ?', [room.id]);
...
app.post('/api/devices', async (req, res) => {
  ...
  try {
    const [result] = await pool.query(
      'INSERT INTO devices (room_id, ha_entity_id, friendly_name, device_type) VALUES (?, ?, ?, ?)',
      [room_id, ha_entity_id, friendly_name, device_type]
    );
    
    const newDevice = { id: result.insertId, ...req.body, position_x: 0.5, position_y: 0.5 };
    
    // Notifica tutti che c'è un nuovo dispositivo
...
app.post('/api/devices/:id/position', async (req, res) => {
  ...
  try {
    await pool.query('UPDATE devices SET position_x = ?, position_y = ? WHERE id = ?', [x, y, id]);
...
app.post('/api/rooms', async (req, res) => {
  ...
  try {
    const [rows] = await pool.query('SELECT MAX(sort_order) as maxOrder FROM rooms');
    ...
    // Notifica socket (opzionale, per aggiornare altri client)
...
// 6. Elimina una stanza (Già che ci siamo, completiamo il lavoro)
